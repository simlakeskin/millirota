...
class ReferenceSessionManager:
    ...
    ÖNEMLİ — Sunucu şeması netleşene kadar iki mod:
      A) JSON dosyası modu (reference_tasks.json): manuel hazırlanmış aralıklar
      B) Otomatik sunucu modu: frame listesinden ref_object_id/ref_image_url
         alanları okunarak çalışma zamanında aralıklar oluşturulur.
    Mod B, sunucu bu alanları gönderdiği anda otomatik devreye girer.
    """
    ...
    def __init__(self, tasks: Optional[List[ReferenceTask]] = None):
        ...
        self._auto_mode = False   # sunucudan otomatik öğrenildi mi

    def add_task(self, task: ReferenceTask):
        self.tasks.append(task)

    def auto_detect_from_frames(self, frames: list, server_url: str = "") -> int:
        """
        Sunucudan gelen frame listesini tarayarak Görev 3 aralıklarını
        otomatik öğren (sunucu bu bilgiyi gönderiyorsa).

        Her frame'de ref_object_id + ref_start_frame + ref_end_frame alanları
        varsa bunları ReferenceTask'e dönüştürür.
        Aynı ref_object_id için tekrar task oluşturmaz.

        Döndürür: tespit edilen yeni görev sayısı
        """
        seen_ids = {t.object_id for t in self.tasks}
        new_count = 0

        for i, frame in enumerate(frames):
            ref_id   = getattr(frame, "ref_object_id",   None)
            ref_url  = getattr(frame, "ref_image_url",   None)
            ref_s    = getattr(frame, "ref_start_frame", None)
            ref_e    = getattr(frame, "ref_end_frame",   None)

            if ref_id is None or ref_id in seen_ids:
                continue

            # ref_image_url göreceli ise tam URL'e çevir
            if ref_url and ref_url.startswith("/") and server_url:
                ref_url = server_url.rstrip("/") + ref_url

            # Aralık bilgisi yoksa o frame'den itibaren tahmin et
            start = ref_s if ref_s is not None else i
            end   = ref_e if ref_e is not None else (i + 200)

            if ref_url:
                # Referans görselini geçici dosyaya indir
                import urllib.request, tempfile, os
                try:
                    tmp = tempfile.NamedTemporaryFile(
                        suffix=".jpg", delete=False,
                        prefix=f"ref_{ref_id}_"
                    )
                    urllib.request.urlretrieve(ref_url, tmp.name)
                    ref_path = tmp.name
                except Exception as e:
                    print(f"[Görev 3] Referans görseli indirilemedi ({ref_url}): {e}")
                    continue
            else:
                print(f"[Görev 3] ref_object_id={ref_id} için görsel URL'i yok, atlanıyor.")
                continue
            ...
            print(f"[Görev 3] Otomatik tespit: object_id={ref_id} | "
                  f"aralık=[{start},{end}] | görsel={ref_path}")
