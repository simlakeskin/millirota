...
Kullanım:
    python main.py --server http://192.168.1.100:5000 --takim 4
    python main.py --server http://127.0.0.25:5000   --takim 4 --model runs/millirota_v1/weights/best.pt
"""
...
try:
    ...
    load_dotenv()  # proje kökündeki .env dosyasını oku (TEAM_NAME, PASSWORD, EVALUATION_SERVER_URL, SESSION_NAME)
except ImportError:
    print("[UYARI] python-dotenv yüklü değil, .env okunamayacak. "
          "Çalıştırın: pip install python-dotenv")
...
class MotionDetector:
    ...
    def update(self, frame: np.ndarray) -> "MotionDetector":
    ...
    def is_moving(self, x1: float, y1: float,
        ...
        """
        if self.prev_gray is None or not self._bg_flow_history:
            return False
...
def run_competition(server_url:  str,
    ...
    print(f"  Takım  : {team_name or takim_id}")
    print(f"  Oturum : {session_name or '(belirtilmedi)'}")
    print(f"  Model  : {model_path or 'Simülasyon'}")
    print(f"{'='*60}\n")

    # ── Modüller ──────────────────────────────────────────────────────────────
    client   = TeknoFestClient(server_url, takim_id,
                               team_name=team_name,
                               password=password,
                               session_name=session_name)
    detector = YOLOv11Detector(model_path or "millirota_v1.pt", conf)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--server", default=os.environ.get("EVALUATION_SERVER_URL", "http://127.0.0.25:5000"),
                        help="Sunucu adresi (.env: EVALUATION_SERVER_URL). Örn: http://havaciliktayapayzeka.teknofest.org:1025/")
    parser.add_argument("--takim",  type=int, default=None,
                        help="(Eski) Takım kullanıcı ID'si — TEAM_NAME kullanılıyorsa gerekmez")
    parser.add_argument("--team-name", default=os.environ.get("TEAM_NAME"),
                        help="Takım kullanıcı adı (.env: TEAM_NAME)")
    parser.add_argument("--password", default=os.environ.get("PASSWORD"),
                        help="Takım şifresi (.env: PASSWORD)")
    parser.add_argument("--session-name", default=os.environ.get("SESSION_NAME"),
                        help="Yarışma oturum adı (.env: SESSION_NAME)")
    ...
    # TEAM_NAME içindeki sayısal takım id'sini (örn. millirota_4634040 -> 4634040)
    # geriye dönük uyumluluk için takim_id olarak da çıkar.
    takim_id = args.takim
    if takim_id is None and args.team_name:
        m = re.search(r"(\d+)$", args.team_name)
        takim_id = int(m.group(1)) if m else 4
    elif takim_id is None:
        takim_id = 4
